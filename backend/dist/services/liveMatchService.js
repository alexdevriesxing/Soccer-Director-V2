...
class LiveMatchService {
    ...
    static createTeamState(club, isHome) {
        return {
            id: club.id,
            name: club.name,
            score: 0,
            players: club.players.map((p, i) => ({
                id: p.player.id,
                name: p.player.name,
    ...
    static getStartingPosition(index, isHome) {
    ...
    static simulateMatchEvent(match) {
        ...
        const getRandomPlayer = (team, positionWeights) => {
    ...
    static getEventDescription(eventType, player1, player2, team, opponent) {
        switch (eventType) {
            case 'goal':
    ...
    static updatePlayerPositions(match) {
        ...
        allPlayers.forEach(player => {
            if (player.hasBall) {
                ...
                const ballDistance = Math.sqrt(Math.pow(ball.x - player.position.x, 2) +
            ...
            if (!ball.holderId) {
                const distanceToBall = Math.sqrt(Math.pow(ball.x - player.position.x, 2) +
                    Math.pow(ball.y - player.position.y, 2));
                if (distanceToBall < 3) {
                    player.hasBall = true;
...
LiveMatchService.matches = new Map();
LiveMatchService.intervals = new Map();
