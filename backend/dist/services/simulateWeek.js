...
exports.simulateWeek = simulateWeek;
exports.triggerWeekSimulation = triggerWeekSimulation;
const client_1 = require("@prisma/client");
const aiManagerService_1 = require("./aiManagerService");
...
function simulateWeek(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            yield generateWeeklyNews(weekNumber);
            ...
            if (weekNumber === 34) {
                ...
                yield handleEredivisiePromotionRelegation();
                yield handlePromotionRelegationBetweenDivisions('Eerste Divisie', 'Tweede Divisie');
                yield handlePromotionRelegationBetweenDivisions('Tweede Divisie', 'Derde Divisie');
                yield handlePromotionRelegationBetweenDivisions('Derde Divisie', 'Vierde Divisie');
                // Add more as needed for lower divisions
                // Optionally: await PromotionRelegationService.handleZaterdagNoordPromotionRelegation();
                yield handleO21PromotionRelegation();
                yield handleJongGraduations();
 ...
 */
function simulateWeekMatches(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
...
function simulateMatch(fixture) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const stadium = (_a = homeClub.facilities) === null || _a === void 0 ? void 0 : _a.find((f) => f.type === 'stadium');
        const stadiumCapacity = (stadium === null || stadium === void 0 ? void 0 : stadium.capacity) || 10000;
        const ticketPrice = (stadium === null || stadium === void 0 ? void 0 : stadium.ticketPrice) || 25;
        ...
        yield updateClubAfterMatch(homeClub, homeGoals, awayGoals);
        yield updateClubAfterMatch(awayClub, awayGoals, homeGoals);
    });
}
/**
 * Calculate team strength based on players, tactics, and home advantage
 */
function calculateTeamStrength(club, isHome) {
    var _a, _b;
    const players = club.players.filter((p) => !p.injured);
    ...
    let strength = players.reduce((sum, p) => sum + p.skill, 0) / players.length;
    // Home advantage
    if (isHome) {
        strength *= 1.1;
    }
    ...
    if (strategy) {
        if (strategy.approach === 'attacking')
            strength += 5;
        if (strategy.approach === 'defensive')
            strength -= 3;
    }
    // Club morale bonus
 ...
 * Simulate goals using Poisson distribution
 */
function simulateGoals(teamStrength, opponentStrength, isHome) {
 ...
 */
function generateMatchEvents(fixture, homeGoals, awayGoals) {
    const events = [];
 ...
 */
function updateClubAfterMatch(club, goalsFor, goalsAgainst) {
    return __awaiter(this, void 0, void 0, function* () {
        let moraleChange = 0;
        let formChange = '';
 ...
 */
function updateWeeklyFinances(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const club of clubs) {
            ...
            const sponsorshipIncome = club.sponsorships.reduce((sum, s) => sum + (s.value / 52), 0); // Weekly
            const tvRightsIncome = 0; // Would be calculated based on league position and TV deals
            // Calculate weekly expenses
            const playerWages = club.players.reduce((sum, p) => sum + p.wage, 0);
            const staffWages = club.staff.reduce((sum, s) => sum + (s.wage || 0), 0);
            const facilityCosts = club.facilities.reduce((sum, f) => sum + f.maintenanceCost, 0);
            ...
            const stadium = club.facilities.find((f) => f.type === 'stadium');
            const ticketPrice = (stadium === null || stadium === void 0 ? void 0 : stadium.ticketPrice) || 25;
            const stadiumCapacity = (stadium === null || stadium === void 0 ? void 0 : stadium.capacity) || 10000;
            ...
            if (ticketPrice > 40 || attendance < 0.6 * stadiumCapacity) {
                yield prisma.boardMember.updateMany({
            ...
            const fanGroups = yield prisma.fanGroup.findMany({ where: { clubId: club.id } });
            for (const fanGroup of fanGroups) {
                if (ticketPrice > 40 || attendance < 0.6 * stadiumCapacity) {
                    yield prisma.fanSentiment.create({
                        data: {
 ...
 */
function calculateGateReceipts(club, weekNumber) {
    var _a;
    // Fetch stadium facility for club
    const stadium = (_a = club.facilities) === null || _a === void 0 ? void 0 : _a.find((f) => f.type === 'stadium');
    const stadiumCapacity = (stadium === null || stadium === void 0 ? void 0 : stadium.capacity) || 10000;
    const ticketPrice = (stadium === null || stadium === void 0 ? void 0 : stadium.ticketPrice) || 25;
    const baseAttendance = 15000;
 ...
 */
function processTransfersAndLoans() {
    return __awaiter(this, void 0, void 0, function* () {
        // Process accepted transfers
        const acceptedTransfers = yield prisma.transfer.findMany({
        ...
        const acceptedLoans = yield prisma.loan.findMany({
            where: { status: 'accepted' },
            include: { player: true }
        });
 ...
 */
function updatePlayerStats(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        const clubs = yield prisma.club.findMany();
        for (const club of clubs) {
            try {
                yield trainingService_1.default.conductTrainingSession(club.id, 'weekly');
            }
            catch (err) {
 ...
 */
function generateWeeklyNews(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
...
function processWeeklyInjuryRecovery() {
    return __awaiter(this, void 0, void 0, function* () {
        const injuries = yield prisma.playerInjury.findMany({
            where: { endDate: { not: null } },
            include: { player: { include: { club: true } } }
        });
        const now = new Date();
        for (const injury of injuries) {
            if (!injury.player || !injury.player.clubId)
                continue;
            // Fetch medical facility level
            const facility = yield prisma.facility.findFirst({ where: { clubId: injury.player.clubId, type: 'medical' } });
            ...
            if (newEnd < end) {
                yield prisma.playerInjury.update({ where: { id: injury.id }, data: { endDate: newEnd } });
            }
            // If injury is over, mark player as recovered
            if (now >= newEnd) {
                yield prisma.player.update({ where: { id: injury.playerId }, data: { injured: false } });
            }
        }
    });
}
// Helper: Simulate matches and collect results
function simulateWeekMatchesWithSummary(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        const fixtures = yield prisma.fixture.findMany({
...
function updatePlayerStatsWithSummary(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        const clubs = yield prisma.club.findMany();
        const training = [];
        ...
        for (const club of clubs) {
            try {
                const result = yield trainingService_1.default.conductTrainingSession(club.id, 'weekly');
                training.push(Object.assign({ club: club.name }, result));
...
function triggerWeekSimulation(weekNumber) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log(`Manually triggering week ${weekNumber} simulation...`);
        yield simulateWeek(weekNumber);
    });
}
